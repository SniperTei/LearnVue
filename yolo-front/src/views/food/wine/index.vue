<template>
  <div class="wine-container">
    ...
    <!-- 酒类列表 -->
    <div class="wines-list" v-loading="loading">
      <el-empty v-if="wines.length === 0" description="暂无酒类" />
      <el-card v-else v-for="wine in wines" :key="wine._id" class="wine-card">
        <div class="wine-header">
          <div class="title-info">
            <h3>{{ wine.name }}</h3>
            <div class="tags">
              <el-tag>{{ getCategoryLabel(wine.type) }}</el-tag>
              <el-tag type="warning">{{ wine.alcoholContent }}%</el-tag>
              <el-tag type="info">{{ wine.brand }}</el-tag>
            </div>
          </div>
          <div class="actions">
            <el-button type="primary" link @click="openEditDialog(wine)">
              <el-icon><Edit /></el-icon>
            </el-button>
            <el-button type="danger" link @click="handleDelete(wine._id)">
              <el-icon><Delete /></el-icon>
            </el-button>
          </div>
        </div>
        <div class="wine-content">
          <div class="image" v-if="wine.imageUrl">
            <el-image 
              :src="wine.imageUrl" 
              fit="cover"
              :preview-src-list="[wine.imageUrl]"
            >
              <template #error>
          ...
          <div class="details">
            <div class="info">
              <span class="info-item">
                ...
                容量: {{ wine.volume }}{{ wine.volumeUnit }}
              </span>
              <span class="info-item">
                <el-icon><Timer /></el-icon>
                创建时间: {{ formatDate(wine.createdAt) }}
              </span>
              <span class="info-item">
            ...
            <div class="description" v-if="wine.description">
